import { Camera, Flag, Map, Maximize2, RadioTower, Route, Video } from 'lucide-react'
...
export function TrackMap({ snapshot, selectedDriver, onSelectDriver }: TrackMapProps) {
    ...
    <section className={`panel track-panel view-${viewMode} camera-${cameraMode}`}>
      <div className="track-panel-header">
        <div>
          ...
          <h2>Silverstone <small>5.891 KM</small></h2>
        </div>
        <div className="track-controls">
          <div className="segment-control">
            <button className={viewMode === 'map' ? 'active' : ''} onClick={() => setViewMode('map')}><Map size={13} /> 2D</button>
            <button className={viewMode === 'track' ? 'active' : ''} onClick={() => setViewMode('track')}><Route size={13} /> TRACK</button>
          </div>
          <button className={`icon-button ${cameraMode === 'broadcast' ? 'active' : ''}`} onClick={() => setCameraMode('broadcast')} title="Broadcast camera"><Video size={15} /></button>
          <button className={`icon-button ${cameraMode === 'onboard' ? 'active' : ''}`} onClick={() => setCameraMode('onboard')} title="Onboard camera"><Camera size={15} /></button>
          <button className="icon-button" title="Fullscreen"><Maximize2 size={15} /></button>
        </div>
      </div>
      ...
      <div className="track-stage">
        ...
        <svg className="circuit-svg" viewBox="0 0 760 430" role="img" aria-label="Live Silverstone circuit map">
          <defs>
            ...
            <linearGradient id="roadGradient" x1="0" y1="0" x2="1" y2="1">
              ...
              <stop offset="0.52" stopColor="#191d23" />
              <stop offset="1" stopColor="#30343b" />
            </linearGradient>
          </defs>
          ...
          {snapshot.drivers.slice().reverse().map((driver) => {
            ...
            return (
              <g
                key={driver.id}
                className={`map-car ${isSelected ? 'selected' : ''}`}
        ...
        <div className="track-card speed-trap-card">
          ...
          <div><small>SPEED TRAP · {selectedDriver.code}</small><b>{Math.round(selectedDriver.speed)} <em>KM/H</em></b></div>
        </div>
