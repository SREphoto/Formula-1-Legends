...
export function getSampleTeamRadio(driverNumber: number): Array<{ time: string; text: string; speaker: string; audioDurationSec: number }> {
  if (driverNumber === 4) {
    return [
      { time: 'LAP 41', speaker: 'Will Joseph (Race Engineer)', text: 'Lando, gap to Max is 2.4s. Mode attack available, let’s use the overtake on Hangar Straight.', audioDurationSec: 4.2 },
      { time: 'LAP 38', speaker: 'Lando Norris', text: 'Tires feel solid mate. Pace is strong, let me know if we need any lift and coast.', audioDurationSec: 3.1 },
      { time: 'LAP 28', speaker: 'Will Joseph (Race Engineer)', text: 'Box this lap, box for Hards. Watch the pit entry line.', audioDurationSec: 2.8 },
    ]
  }
  return [
    { time: 'LAP 40', speaker: 'Gianpiero Lambiase (GP)', text: 'Max, Norris is within DRS threat. Recharge available out of turn 7.', audioDurationSec: 3.5 },
    { time: 'LAP 32', speaker: 'Max Verstappen', text: 'Front left is starting to slide a bit in high speed.', audioDurationSec: 2.4 },
    { time: 'LAP 25', speaker: 'Gianpiero Lambiase (GP)', text: 'Box box, hard compound fitted.', audioDurationSec: 1.9 },
  ]
}
